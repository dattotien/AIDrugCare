...
class HMGRLService:
    def __init__(self, model_path, data_path, device="cpu"):
        ...
        # Load mapping files
        with open(os.path.join(data_path, "drugbankid2id.json"), "r") as f:
            self.drugbankid2id = json.load(f)
        with open(os.path.join(data_path, "id2drugbankid.json"), "r") as f:
            self.id2drugbankid = json.load(f)
        with open(os.path.join(data_path, "name_to_drugbank_id.json"), "r") as f:
            self.name_to_dbid = json.load(f)
        with open(os.path.join(data_path, "drug_interaction.json"), "r") as f:
            self.inter_dict = json.load(f)

        # Default tensor
        self.N_three_attribute = torch.tensor(
            [2033, 1589, 285], dtype=torch.long, device=self.device
        )
    ...
    def load_model(self, path):
        ...
        self.model = HMGRL(
            6000, 99, 1000, self.N_three_attribute, args
        ).to(self.device)
    ...
    def load_data(self, path):
        ...
        X_vector = np.load(os.path.join(path, "X_vector.npy"))
        self.drug_coding = torch.tensor(
            np.load(os.path.join(path, "drug_coding.npy")),
            dtype=torch.float,
            device=self.device,
        )
        ...
        self.X_three_vector = torch.tensor(
            X_three_vector, dtype=torch.float, device=self.device
        )
        self.N_three_attribute = torch.tensor(
            [2033, 1589, 285], dtype=torch.int, device=self.device
        )
    ...
    def prepare_adj(self):
        ...
        self.adj, self.n_drugs = adj_Heter_gene(
            self.DDI_edge, self.X_vector, self.event_num, self.new_label
        )
